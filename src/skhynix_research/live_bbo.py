...
def fetch_product_metadata(symbols: dict[str, str], root: Path = METADATA_ROOT,
                           client: httpx.Client | None = None) -> dict[str, ProductMetadata]:
    ...
    try:
        for exchange in EXCHANGES:
            raw_file = root / "raw" / f"{stamp}-{exchange}.json"
    ...
    frame.to_parquet(root / "latest.parquet", index=False)
...
class BBOStorage:
    ...
    @classmethod
    def _flush_raw(cls, records: list[tuple[Path, str]]):
        grouped: dict[Path, list[str]] = defaultdict(list)
        for path, text in records: grouped[path].append(text)
        for path, texts in grouped.items(): cls._append(path, "".join(texts))
...
class WebSocketVenue:
    def __init__(self, adapter: VenueAdapter, symbol: str, storage: BBOStorage,
                 metadata: ProductMetadata, monitor: CollectorMonitor, settings: dict):
        self.adapter, self.symbol, self.storage = adapter, symbol, storage
        self.metadata, self.monitor, self.settings = metadata, monitor, settings
...
async def run_live_bbo(duration_seconds: float | None = None):
    ...
    venues = [WebSocketVenue(spec, symbols[name], storage, metadata[name], monitor, settings)
